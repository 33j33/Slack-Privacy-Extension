    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        try {
            ...
            if (toggleCommand && toggleCommand.shortcut) {
                ...
                if (macShortcutKey)
                    macShortcutKey.textContent = toggleCommand.shortcut;
                if (windowsShortcutKey)
                    windowsShortcutKey.textContent = toggleCommand.shortcut;
            }
        }
        catch (error) {
        ...
        browserAPI.runtime.onMessage.addListener((message, sender, sendResponse) => {
            if (message.type === 'settingsUpdated') {
                ...
                if (settings.enabled !== undefined && settings.enabled !== null) {
                    const mainToggle = document.getElementById('mainToggle');
                    const settingsPanel = document.getElementById('settingsPanel');
                    currentSettings = Object.assign(Object.assign({}, currentSettings), { enabled: settings.enabled });
                    if (mainToggle) {
                        mainToggle.checked = currentSettings.enabled;
                    }
                    if (settingsPanel) {
